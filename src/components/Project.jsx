...
function Project(props) {
    return (
        <div className="w-full h-[360px] bg-zinc-700 rounded overflow-hidden shadow-lg relative">
            ...
            <p className="text-zinc-300 font-roboto pl-3 pr-3 mt-2">
                {props.desc}
                {props.moreInfo && 
                    <span> <span className='text-gray-400 cursor-pointer' tabIndex={0} onClick={() => props.setMoreInfo(props)}>Show more...</span></span>
                }
            </p>

            {props.url &&
                <a className="ml-3 bottom-3 absolute flex items-center w-fit" target="_blank" href={props.url}>
