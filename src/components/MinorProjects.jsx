        ...
        <div>
            ...
            <div className="grid m-20 gap-4 grid-cols-[repeat(auto-fill,19rem)] sm:grid-cols-[repeat(auto-fill,24rem)] justify-center">
                {minorProjects.map((project) => (
                    <Project key={project.name} setMoreInfo={props.setMoreInfo} projectName={project.name} year={project.year} desc={project.description} showMore={project.showMore} url={project.url} bannerImage={project.banner} icon={project.icon}/>
                ))}
            </div>
        </div>
    )
